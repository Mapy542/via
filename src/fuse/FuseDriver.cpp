    ...
    if (!file.open(QIODevice::ReadOnly)) {
        ...
        if (drv->m_fileCache) {
            ...
            if (!newPath.isEmpty()) {
                ...
                if (file.open(QIODevice::ReadOnly)) {
                    qWarning() << "FuseDriver: fuseRead recovered stale cache path for" << openFile.fileId;
                } else {
...
int FuseDriver::fuseTruncate(const char* path, off_t size, struct fuse_file_info* fi) {
    ...
    if (fi) {
        ...
        if (openFileOpt && openFileOpt->fileId == meta.fileId) {
            cachePath = openFileOpt->cachePath;
            openedViaHandle = true;
        }
    }

    // M2 fix: Download file if not cached, so truncate works correctly.
    // TODO: Collapse truncate/write/fsync onto one snapshot-based staging path
    // so resize-heavy save patterns share exactly the same upload lifecycle.
    if (cachePath.isEmpty()) {
