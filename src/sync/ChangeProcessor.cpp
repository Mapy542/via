...
void ChangeProcessor::determineAndQueueActions(const ChangeQueueItem& change) {
    ...
    if (remoteNoDelete &&
        (action.actionType == SyncActionType::DeleteRemote || action.actionType == SyncActionType::TrashRemote)) {
...
bool ChangeProcessor::buildSyncActionForChange(const ChangeQueueItem& change, SyncActionItem& action) const {
    ...
    switch (change.changeType) {
        ...
        case ChangeType::Delete:
            action.actionType =
                change.origin == ChangeOrigin::Local ? SyncActionType::TrashRemote : SyncActionType::DeleteLocal;
            return true;
